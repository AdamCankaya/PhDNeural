...
DEFAULT_STATE_PATH = ROOT / ".epi-decon_phd-github-sync.json"
...
def sync_tasks(
    ...
    repo_arg = repo if config.get("GITHUB_PROJECT_SCOPE") == "repository" else None
    repository = client.get_repository(owner, repo)
    project = client.get_project(owner, project_number, repo=repo_arg)
    ...
    for index, task in enumerate(tasks, start=1):
        ...
        if existing:
            ...
            created += 1
            print(f"  Created issue #{issue_number}".encode("utf-8", "replace").decode("utf-8"))
            time.sleep(0.3)
